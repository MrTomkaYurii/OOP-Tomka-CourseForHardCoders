...
public class AppointmentManager
{
    ...
    public Appointment[] GetByPatient(int patientId)
    {
        ...
        int idx = 0;
        for (int i = 0; i < _count; i++)
            if (_appointments[i].PatientId == patientId) result[idx++] = _appointments[i];
        return result;
    }
    ...
    public Appointment[] GetByDoctor(int doctorId)
    {
        ...
        int idx = 0;
        for (int i = 0; i < _count; i++)
            if (_appointments[i].DoctorId == doctorId) result[idx++] = _appointments[i];
        return result;
    }

    public Appointment[] GetByDate(int year, int month, int day) => GetByDate(new DateTime(year, month, day));
    ...
    public Appointment[] GetUpcoming()
    {
        ...
        int idx = 0;
        for (int i = 0; i < _count; i++)
            if (_appointments[i].IsUpcoming) result[idx++] = _appointments[i];
        return result;
    }
    ...
    {
        if (appointments.Length == 0) { Console.WriteLine("Записів не знайдено."); return; }
