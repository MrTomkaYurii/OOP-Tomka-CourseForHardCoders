...
public class PatientManager
{
    ...
    public Patient[] FindByName(string query)
    {
        ...
        // Другий прохід: заповнюємо результат
        Patient[] result = new Patient[matchCount];
        int idx = 0;
        for (int i = 0; i < _count; i++)
        {
    ...
    public void DisplayStats()
    {
        ...
        double totalAge = 0;
        int minAge = _patients[0].Age;
        int maxAge = _patients[0].Age;
        int minIdx = 0;
        int maxIdx = 0;
        int adultsCount = 0;
        ...
        for (int i = 0; i < _count; i++)
        {
            ...
            if (_patients[i].Age < minAge)
            {
                minAge = _patients[i].Age;
                minIdx = i;
            }
            if (_patients[i].Age > maxAge)
            {
                maxAge = _patients[i].Age;
                maxIdx = i;
            }
