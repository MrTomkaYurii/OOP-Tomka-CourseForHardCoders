...
Це стан гілки `main` перед початком роботи. Тренажери `Lab01/` і `Lab02/` лишились
на своїх гілках і сюди не потрапили:
...
консольне меню. Структуру проєкту **наприкінці** лаби (з позначками, що і в якій
задачі створюється) ви знайдете в розділі «Структура проєкту наприкінці лаби»
перед перевіркою.
...
Повна схема файлів — у розділі «Структура проєкту наприкінці лаби» (перед перевіркою).
`Program.cs` — спільна точка входу: меню й тестовий код, який росте разом із задачами.
...
Так має виглядати ваш репозиторій, коли всі 8 завдань виконано:
...
└── ClinicApp/
    ...
    ├── Patient.cs                   🆕 Т1
    ├── Doctor.cs                    🆕 Т2
    ├── PatientManager.cs            🆕 Т3
    ├── DoctorManager.cs             🆕 Т4
    ├── Appointment.cs               🆕 Т5
    ├── AppointmentManager.cs        🆕 Т6
    ├── Clinic.cs                    🆕 Т7
    └── GrowablePatientManager.cs    🆕 Т8
```

**Легенда:** 🆕 — новий файл · ✏ — змінено вміст · Т*n* — номер задачі, у якій ви працюєте з файлом.

Назви файлів наведено для домену «клініка»; у власному домені назви ваші — важливе розташування й те, що всі файли лежать в одній теці `ClinicApp/` (підпапки з'являться в Лабі 05).
