...
Код повинен **використовувати тільки конструкції з лаб ≤ поточної**.
Кожна лаба вводить нове. Якщо конструкція ще не введена — вона заборонена в еталонному коді.

> **Іменування (як в інструкціях `labs/`):**
...
| `null`, `if (x == null)` / `if (x != null)` | 03 | явні перевірки, без `?.`/`??` |
| Nullable-анотація `T?` на результаті, `!` після `ReadLine()` | 03 | коротке пояснення в самій лабі |
| Масив об'єктів `Patient[]` + ручний лічильник | 03 | замість `List<T>` (той — Лаба 09) |
| `const` поле | 03 | для MaxCapacity |
| `int.TryParse(s, out x)` — споживання API з `out` | 03 | свої `out`-методи — Лаба 04 |
...
### Lab 01 — Основи C# (тека `Lab01/`, гілка `Lab-01`, у `main` НЕ зливається)
...
- Масиви (крім Задачі 7 — там одновимірний як розігрів), класи, колекції будь-які
- кортежі й `(a,b)=(b,a)` swap (модуль 8 лекцій); патерн `or` у `switch` —
  Задача 6 будує вибір через `(card % 10) / 2` + звичайні гілки

---

### Lab 02 — Масиви (тека `Lab02/`, гілка `Lab-02`, у `main` НЕ зливається)
...
- кортежі: swap лише через тимчасову змінну (Задачі 2, 7)

---

### Lab 03 — Визначення класів (тека `ClinicApp/`, гілка `Lab-03`, **зливається в `main`** — перше злиття)
...
- Nullable-анотація `T?` на типі результату (`Patient? FindById(...)`) — з коротким
  поясненням у самій лабі; оператор `!` після `Console.ReadLine()`
- `const` поле в класі
...
- `DateTime`: `.Today`, `.Now`, `.Year`, `.Hour`, `.Date`, `.AddYears()`, `.AddMinutes()`
  (глибокий розбір `DateTime` — модуль 12 лекцій; тут — точково, з посиланням на docs)

**Колекції в Lab 03:**
- Тільки `T[]` з ручним лічильником `_count`. Задача 8 — зростаючий масив
  (подвоєння + копіювання циклом), **без згадок `List<T>`**.

**Заборонено:**
- `List<T>`, `Dictionary<K,V>`, `Queue<T>` та будь-які generic-колекції — Лаба 09
- generic-класи/методи будь-які
- Будь-який LINQ — Лаба 14
- `TimeOnly`, `DateOnly` (занадто специфічні)
- Скорочені оператори `?.` та `??` — Лаба 04 (тут явні `== null` / `!= null`)
- `is not null` та інші патерни зіставлення — значно пізніше
- `IEnumerable<T>` як тип параметра
- `abstract`, `interface`, `enum`, `struct`, `static class`
- кортежі; `int.TryParse` з `out` у меню — дозволено як *споживання* API
  (свої `out`-методи — Лаба 04); за бажанням — `int.Parse` з приміткою «обробка
  помилок у Лабі 05»

---

### Lab 04 — Члени класу (тека `ClinicApp/`, гілка `Lab-04`, зливається в `main`)
