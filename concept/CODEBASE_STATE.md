...
Цей файл відстежує, що конкретно живе в проєкті після кожної лаби.
...
- `override ToString()`
- Методи екземпляра
- `null`, `if (x == null)`, `if (x != null)` — **тільки явні перевірки**, без `?.`/`??`
- Nullable-анотація `T?` на результаті пошуку (`Patient? FindById(...)`),
  `!` після `Console.ReadLine()` — з коротким поясненням у самій лабі
- Масив об'єктів з ручним лічильником; **Задача 8 — зростаючий масив без згадок `List<T>`**
...
- `MedicalRecordManager.FindById` та індексатор повертають `null!` — інструкція: `MedicalRecord?`.
- Лабораторні дані: `new LabResult(…, 6.2, …)` у тестових даних виводиться як `6,2` на uk-UA; після додавання `InvariantCulture` — `6.2`.
- Пастка `InvariantCulture` + `double.TryParse("6,2")` = `62` (кома — роздільник розрядів) — у еталоні не відображена, описана лише в інструкції.
- Побічний ефект глобальної `InvariantCulture`: «Середній вік» у статистиці пацієнтів (Лаба 03) виводиться `29.2` замість `29,2` (перевірено прогоном; дати не змінюються — формати явні).
...
- `BillingManager`: `GetAllUnpaid`/`GetUnpaidByPatient` дублюють фільтр — інструкція радить один приватний метод (`Appointment[]` → `IPayable[]`).
- Головне меню: в еталоні рядки пунктів 1–6 мають 47 символів при рамці 48 — права межа `║` з'їжджає; в інструкції наведено вирівняний варіант (усі рядки по 48).
- Суми: `ToString("F2")` на uk-UA дає `300,00`; при `InvariantCulture` (Лаба 06) — `300.00`.
- Слоти: «вільні» лише за розкладом, записи лікаря не враховуються (в інструкції зазначено прямо).
- Скасування: `Appointment.Cancel` дозволяє скасувати вже **оплачений** запис (оплата «зависає») — в інструкції винесено як питання для роздумів, код не змінюється.
