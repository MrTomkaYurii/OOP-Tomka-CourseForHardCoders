...
- Атомарні Markdown-сторінки сайту зберігаються в `lectures/sections/*.md`.
- Проміжні великі Markdown-файли після первинного переносу зберігаються в `lectures/_migration/source-chunks/*.md`.
...
- Рисунки витягнуті в `lectures/_assets/_docx`; при перенесенні на сайт вони мають бути доступні як публічні assets.
...
Ці великі проміжні файли перенесено в `lectures/_migration/source-chunks/`.
...
- Великі файли `lectures/_migration/source-chunks/01-*.md` ... `lectures/_migration/source-chunks/17-*.md` залишаються як первинні перенесені блоки-джерела для звірки.
- Файли в `lectures/_combined/` залишаються службовими зібраними копіями, які використовувалися під час нарізання матеріалу на атомарні сторінки.
- У сайт інтегровано атомарні сторінки до кінця розділу `14. Рефлексія`, а також службову сторінку `15.1. Для нотаток та видавничі дані`. Нумерацію сторінок нормалізовано за фактичним порядком матеріалу в межах кожного розділу, без пропусків і дублювання номерів. Ненумерований у DOCX блок `Патерни списків` інтегровано як підрозділ `9.6`. Матеріал DOCX перенесено до видавничих даних включно.
